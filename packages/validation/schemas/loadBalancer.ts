...
const lbRuleSchema = z.object({
    ...
    if (['HTTP', 'HTTPS'].includes(data.protocol_type) && !data.l7_health_check_path) {
        return false;
    }
    return true;
}, {
    message: '17 health check path는 HTTP, HTTPS 프로토콜에서 필수입니다.',
    path: ['l7_health_check_path'],
}).refine((data) => {
    if (['SSL', 'HTTPS'].includes(data.protocol_type) && !data.certificate_name) {
        return false;
    }
    return true;
}, {
